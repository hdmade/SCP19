...
( function( $, wpLinkL10n, wp ) {
	...
	wpLink = {
		...
		init: function() {			
			...
			inputs.attrs.class = $( '#wp-link-class' );
			inputs.attrs.id = $( '#wp-link-id' );
			inputs.attrs.nofollow = $( '#wp-link-nofollow' );
			inputs.attrs.title = $( '#wp-link-title' );
		...
		mceRefresh: function( searchStr, text ) {
			var linkText, href, _class, id, rel,
				linkNode = getLink(),
			...
			if ( linkNode ) {
				...
				class_ = editor.dom.getAttrib( linkNode, 'class' );
				title = editor.dom.getAttrib( linkNode, 'title' );
				id = editor.dom.getAttrib( linkNode, 'id' );
				rel = editor.dom.getAttrib( linkNode, 'rel' );
				...
				inputs.attrs.class.val( class_ );
				inputs.attrs.title.val( title );
				inputs.attrs.id.val( id );
				inputs.attrs.nofollow.val( id );
				inputs.attrs.nofollow.prop( 'checked', 'nofollow' === editor.dom.getAttrib( linkNode, 'rel' ) );
		...
		getAttrs: function() {
			...
			return {
				...
				rel: inputs.attrs.nofollow.prop( 'checked' ) ? 'nofollow' : '',
				class: $.trim( inputs.attrs.class.val() ),
				title: $.trim( inputs.attrs.title.val() ),
				id: $.trim( inputs.attrs.id.val() )
			};
		},
	...
	$( document ).ready( 
		function(){
			jQuery( '#link-options' ).append( '<div class="link-nofollow"><label><span></span><input type="checkbox" id="wp-link-nofollow" /> rel="nofollow"</label></div>' );
			jQuery( '#link-options' ).append( '<div class="wp-link-title"><label><span>title</span><input id="wp-link-title" type="text" /></label></div>' );
			jQuery( '#link-options' ).append( '<div class="wp-link-class"><label><span>class</span><input id="wp-link-class" type="text" /></label></div>' );
			jQuery( '#link-options' ).append( '<div class="wp-link-id"><label><span>id</span><input id="wp-link-id" type="text" /></label></div>' );
			wpLink.init();
		}
		);
